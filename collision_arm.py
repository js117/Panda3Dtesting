# Collision Attempt Panda3D # 
# check out collisions-beginner step 8 demo file 
...
def collider_on_joint(node_path, scale=1.):
  objNP=NodePath(PandaNode("phisicsobj"))
  objAN=ActorNode("objactnode")
  objANP=objNP.attachNewNode(objAN)
  objmodel=node_path # this line changed from above template
  print(node_path)
  objmodel.reparentTo(objANP)
  #objmodel.setScale(scale)
  objCollider = objANP.attachNewNode(CollisionNode('objcnode'))
  objCollider.node().addSolid(CollisionSphere(0,0,0, 1*scale))
  base.physicsMgr.attachPhysicalNode(objAN)
  collisionHandler.addCollider(objCollider, objANP)
  base.cTrav.addCollider(objCollider, collisionHandler)
...
smiley=phyball_dispenser('smiley', scale=0.5) 		# 'smiley'

# Let's add some of our stuff: 

mouseLook = FirstPersonCamera(base, base.cam, render)

#robot_arm = actor_dispenser("models/arm7.egg", scale=1.)

robot_arm = ArmActor(base, "models/arm7.egg")
robot_arm.m.reparentTo(base.render)

Je = [0,0,0,0,0,0]
Je[0] = robot_arm.m.exposeJoint(None, 'modelRoot', 'Bone.001')
Je[1] = robot_arm.m.exposeJoint(None, 'modelRoot', 'Bone.002')
Je[2] = robot_arm.m.exposeJoint(None, 'modelRoot', 'Bone.004')
Je[3] = robot_arm.m.exposeJoint(None, 'modelRoot', 'Bone.007')
Je[4] = robot_arm.m.exposeJoint(None, 'modelRoot', 'Bone.008')
Je[5] = robot_arm.m.exposeJoint(None, 'modelRoot', 'Bone.009')
'''
...
'''
for j in range(0,len(Je)): 
	joint_col_np = collider_on_joint(Je[j], scale=1.)
	print("Je["+str(j)+"] position: "+ str(Je[j].getPos()))
	print("joint_col_np position: "+ str(joint_col_np.getPos()))
	joint_col_np.setPos(Je[j].getPos())
